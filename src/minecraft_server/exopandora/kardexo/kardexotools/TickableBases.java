...
public class TickableBases implements ITickable
{
	...
	public void update()
	{
		for(Property base : Config.BASES.getData().values())
		{
			...
			for(EntityPlayerMP player : this.server.getPlayerList().getPlayers())
			{
				...
				Set<String> visitors = this.BASE_VISITORS.get(base);
				...
				if(visitors.contains(player.getName()))
				{
					if(!inside)
					{
						...
						this.BASE_VISITORS.put(base, visitors);
	...
	{
		if(!base.isOwner(player.getName()))
		{
			switch(access)
			{
				case ENTER:
					this.server.logInfo(player.getName() + " has entered base with id " + base.getTitle());
					break;
				case LEAVE:
					this.server.logInfo(player.getName() + " has left base with id " + base.getTitle());
					break;
			}
